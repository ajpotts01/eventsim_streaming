...
obt_events as (
select 
    case when stg_listen_events.topic_name = 'listen_events' then 'listen_event'
        when stg_auth_events.topic_name = 'auth_events' then 'auth_event'
        when stg_page_view_events.topic_name = 'page_view_events' then 'page_view_event'
        when stg_status_change_events.topic_name = 'status_change_events' then 'status_change_event' 
    end as event_type,
